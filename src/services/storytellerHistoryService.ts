...
const HISTORY_COLLECTION = 'storytellerHistory';

export async function addHistoryItem(
  itemData: Omit<StorytellerHistoryItem, 'id' | 'createdAt' | 'updatedAt' | 'userId'>
): Promise<string> {
...
export async function updateHistoryItem(
  id: string,
  data: Partial<Omit<StorytellerHistoryItem, 'id' | 'userId' | 'createdAt'>>
): Promise<void> {
    const user = auth.currentUser;
    if (!user) throw new Error("User not authenticated.");
    try {
...
export async function getHistoryItems(): Promise<StorytellerHistoryItem[]> {
  const user = auth.currentUser;
  if (!user) return [];
  try {
    const q = query(
    ...
    const items = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as StorytellerHistoryItem));
