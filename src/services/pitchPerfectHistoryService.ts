...
const HISTORY_COLLECTION = 'pitchPerfectHistory';

export async function addHistoryItem(
  itemData: Omit<PitchPerfectHistoryItem, 'id' | 'createdAt' | 'updatedAt' | 'userId'>
): Promise<string> {
...
export async function updateHistoryItem(
  id: string,
  data: Partial<Omit<PitchPerfectHistoryItem, 'id' | 'userId'>>
): Promise<void> {
    const user = auth.currentUser;
    if (!user) throw new Error("User not authenticated.");
    try {
...
export async function getHistoryItems(): Promise<PitchPerfectHistoryItem[]> {
  const user = auth.currentUser;
  if (!user) return [];
  try {
    const q = query(
    ...
    const items = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as PitchPerfectHistoryItem));
