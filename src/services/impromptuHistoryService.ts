...
const HISTORY_COLLECTION = 'impromptuHistory';

export async function addHistoryItem(
  itemData: Omit<ImpromptuHistoryItem, 'id' | 'createdAt' | 'updatedAt' | 'userId'>
): Promise<string> {
...
export async function updateHistoryItem(
  id: string,
  data: Partial<Omit<ImpromptuHistoryItem, 'id' | 'userId'>>
): Promise<void> {
    const user = auth.currentUser;
    if (!user) throw new Error("User not authenticated.");
    try {
...
export async function getHistoryItems(): Promise<ImpromptuHistoryItem[]> {
  const user = auth.currentUser;
  if (!user) return [];
  try {
    const q = query(
