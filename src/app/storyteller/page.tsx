...
import { Loader2, Mic, StopCircle, Waves, Image as ImageIcon, History, Play, Lightbulb } from 'lucide-react';
...
export default function StorytellerPage() {
  ...
  const handleGetImages = async () => {
    ...
    resetState();
    try {
      const { images: newImages } = await generateStoryImages();
      setImages(newImages);
              ...
              <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {images.map((img, idx) => (
                  <div key={idx} className="relative aspect-square">
                    <Image src={img} alt={`Story prompt image ${idx+1}`} fill objectFit="cover" className="rounded-lg shadow-md" />
                  </div>
                ))}
              </CardContent>
            </Card>
